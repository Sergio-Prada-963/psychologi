...
class Camper extends Conectar{
    ...
    public function get_camper(){
        try {
            ...
            parent::set_name();
            $stm=$conectar->prepare("SELECT * FROM pacientes");
            $stm->execute();
    ...
    public function get_camper_id($id){
        try {
            $conectar=parent::Conexion();
            parent::set_name();
            $stm=$conectar->prepare("SELECT * FROM pacientes WHERE id=?");
            $stm->bindValue(1,$id);
            $stm->execute();
            return $stm->fetchAll(PDO::FETCH_ASSOC);
        } catch (Exeption $e) {
    ...
    public function insert_camper($id,$imagen,$nombre,$edad, $promedio, $nivelCAmpus, $nivelIngles, $especialidad, $direccion, $celular, $ingles, $Ser,$Review, $Skills, $Asitencia ){
        $conectar=parent::Conexion();
        parent::set_name();
        $stm="INSERT INTO pacientes(id,imagen,nombre,edad,promedio,nivelCAmpus,nivelIngles, especialidad, direccion, celular, ingles, Ser, Review, Skills, Asitencia) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public function update_camper($id,$imagen,$nombre,$edad, $promedio, $nivelCAmpus, $nivelIngles, $especialidad, $direccion, $celular, $ingles, $Ser,$Review, $Skills, $Asitencia){
        $conectar=parent::conexion();
        parent::set_name();
        $sql="UPDATE pacientes set imagen=? , nombre=? ,edad=?, promedio=?, nivelCAmpus=?, nivelIngles=?, especialidad=? ,direccion=? , celular=?, ingles=?, Ser=?,  Review=?, Skills=?,  Asitencia=?  WHERE id=?";
